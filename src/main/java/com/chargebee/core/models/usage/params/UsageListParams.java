/*
 * This file is auto-generated by Chargebee.
 * For more information on how to make changes to this file, please see the README.
 * Reach out to dx@chargebee.com for any questions.
 * Copyright 2025 Chargebee Inc.
 */

package com.chargebee.core.models.usage.params;

import com.chargebee.internal.Recommended;

import java.util.Collections;
import java.util.LinkedHashMap;
import java.util.Map;

public final class UsageListParams {

  private final Map<String, Object> queryParams;

  private UsageListParams(UsageListBuilder builder) {
    this.queryParams = Collections.unmodifiableMap(new LinkedHashMap<>(builder.queryParams));
  }

  /** Get the query parameters for this request. */
  public Map<String, Object> toQueryParams() {
    return queryParams;
  }

  public UsageListBuilder toBuilder() {
    UsageListBuilder builder = new UsageListBuilder();
    builder.queryParams.putAll(queryParams);
    return builder;
  }

  /** Create a new builder for UsageListParams. */
  @Recommended(reason = "Preferred for reusability, validation, and LLM-friendliness")
  public static UsageListBuilder builder() {
    return new UsageListBuilder();
  }

  public static final class UsageListBuilder {
    private final Map<String, Object> queryParams = new LinkedHashMap<>();

    private UsageListBuilder() {}

    public UsageListBuilder limit(Integer value) {
      queryParams.put("limit", value);
      return this;
    }

    public UsageListBuilder offset(String value) {
      queryParams.put("offset", value);
      return this;
    }

    public IdFilter id() {
      return new IdFilter("id", this);
    }

    public SubscriptionIdFilter subscriptionId() {
      return new SubscriptionIdFilter("subscription_id", this);
    }

    public UsageDateFilter usageDate() {
      return new UsageDateFilter("usage_date", this);
    }

    public UpdatedAtFilter updatedAt() {
      return new UpdatedAtFilter("updated_at", this);
    }

    public ItemPriceIdFilter itemPriceId() {
      return new ItemPriceIdFilter("item_price_id", this);
    }

    public InvoiceIdFilter invoiceId() {
      return new InvoiceIdFilter("invoice_id", this);
    }

    public SourceFilter source() {
      return new SourceFilter("source", this);
    }

    public SortBySortBuilder sortBy() {
      return new SortBySortBuilder("sort_by", this);
    }

    public UsageListParams build() {
      return new UsageListParams(this);
    }

    // Field-specific filter classes for fluent API

    public static final class IdFilter {
      private final String fieldName;
      private final UsageListBuilder builder;

      IdFilter(String fieldName, UsageListBuilder builder) {
        this.fieldName = fieldName;
        this.builder = builder;
      }

      public UsageListBuilder is(String value) {
        builder.queryParams.put(fieldName + "[is]", value);
        return builder;
      }

      public UsageListBuilder isNot(String value) {
        builder.queryParams.put(fieldName + "[is_not]", value);
        return builder;
      }

      public UsageListBuilder startsWith(String value) {
        builder.queryParams.put(fieldName + "[starts_with]", value);
        return builder;
      }
    }

    public static final class SubscriptionIdFilter {
      private final String fieldName;
      private final UsageListBuilder builder;

      SubscriptionIdFilter(String fieldName, UsageListBuilder builder) {
        this.fieldName = fieldName;
        this.builder = builder;
      }

      public UsageListBuilder is(String value) {
        builder.queryParams.put(fieldName + "[is]", value);
        return builder;
      }

      public UsageListBuilder isNot(String value) {
        builder.queryParams.put(fieldName + "[is_not]", value);
        return builder;
      }

      public UsageListBuilder startsWith(String value) {
        builder.queryParams.put(fieldName + "[starts_with]", value);
        return builder;
      }
    }

    public static final class UsageDateFilter {
      private final String fieldName;
      private final UsageListBuilder builder;

      UsageDateFilter(String fieldName, UsageListBuilder builder) {
        this.fieldName = fieldName;
        this.builder = builder;
      }

      public UsageListBuilder after(String timestamp) {
        builder.queryParams.put(fieldName + "[after]", timestamp);
        return builder;
      }

      public UsageListBuilder before(String timestamp) {
        builder.queryParams.put(fieldName + "[before]", timestamp);
        return builder;
      }

      public UsageListBuilder on(String timestamp) {
        builder.queryParams.put(fieldName + "[on]", timestamp);
        return builder;
      }

      public UsageListBuilder between(String start, String end) {
        builder.queryParams.put(fieldName + "[between]", "[" + start + "," + end + "]");
        return builder;
      }
    }

    public static final class UpdatedAtFilter {
      private final String fieldName;
      private final UsageListBuilder builder;

      UpdatedAtFilter(String fieldName, UsageListBuilder builder) {
        this.fieldName = fieldName;
        this.builder = builder;
      }

      public UsageListBuilder after(String timestamp) {
        builder.queryParams.put(fieldName + "[after]", timestamp);
        return builder;
      }

      public UsageListBuilder before(String timestamp) {
        builder.queryParams.put(fieldName + "[before]", timestamp);
        return builder;
      }

      public UsageListBuilder on(String timestamp) {
        builder.queryParams.put(fieldName + "[on]", timestamp);
        return builder;
      }

      public UsageListBuilder between(String start, String end) {
        builder.queryParams.put(fieldName + "[between]", "[" + start + "," + end + "]");
        return builder;
      }
    }

    public static final class ItemPriceIdFilter {
      private final String fieldName;
      private final UsageListBuilder builder;

      ItemPriceIdFilter(String fieldName, UsageListBuilder builder) {
        this.fieldName = fieldName;
        this.builder = builder;
      }

      public UsageListBuilder is(String value) {
        builder.queryParams.put(fieldName + "[is]", value);
        return builder;
      }

      public UsageListBuilder isNot(String value) {
        builder.queryParams.put(fieldName + "[is_not]", value);
        return builder;
      }

      public UsageListBuilder startsWith(String value) {
        builder.queryParams.put(fieldName + "[starts_with]", value);
        return builder;
      }
    }

    public static final class InvoiceIdFilter {
      private final String fieldName;
      private final UsageListBuilder builder;

      InvoiceIdFilter(String fieldName, UsageListBuilder builder) {
        this.fieldName = fieldName;
        this.builder = builder;
      }

      public UsageListBuilder is(String value) {
        builder.queryParams.put(fieldName + "[is]", value);
        return builder;
      }

      public UsageListBuilder isNot(String value) {
        builder.queryParams.put(fieldName + "[is_not]", value);
        return builder;
      }

      public UsageListBuilder startsWith(String value) {
        builder.queryParams.put(fieldName + "[starts_with]", value);
        return builder;
      }

      public UsageListBuilder isPresent(boolean value) {
        builder.queryParams.put(fieldName + "[is_present]", String.valueOf(value));
        return builder;
      }
    }

    public static final class SourceFilter {
      private final String fieldName;
      private final UsageListBuilder builder;

      SourceFilter(String fieldName, UsageListBuilder builder) {
        this.fieldName = fieldName;
        this.builder = builder;
      }

      public UsageListBuilder is(String value) {
        builder.queryParams.put(fieldName + "[is]", value);
        return builder;
      }

      public UsageListBuilder isNot(String value) {
        builder.queryParams.put(fieldName + "[is_not]", value);
        return builder;
      }

      public UsageListBuilder in(String... values) {
        builder.queryParams.put(fieldName + "[in]", "[" + String.join(",", values) + "]");
        return builder;
      }

      public UsageListBuilder notIn(String... values) {
        builder.queryParams.put(fieldName + "[not_in]", "[" + String.join(",", values) + "]");
        return builder;
      }
    }

    // Sort builder classes for fluent API

    public static final class SortBySortBuilder {
      private final String fieldName;
      private final UsageListBuilder builder;

      SortBySortBuilder(String fieldName, UsageListBuilder builder) {
        this.fieldName = fieldName;
        this.builder = builder;
      }

      public SortDirection usage_date() {
        return new SortDirection(fieldName + "[usage_date]", builder);
      }

      public SortDirection updated_at() {
        return new SortDirection(fieldName + "[updated_at]", builder);
      }
    }

    public static final class SortDirection {
      private final String paramName;
      private final UsageListBuilder builder;

      SortDirection(String paramName, UsageListBuilder builder) {
        this.paramName = paramName;
        this.builder = builder;
      }

      public UsageListBuilder asc() {
        builder.queryParams.put(paramName + "[asc]", "true");
        return builder;
      }

      public UsageListBuilder desc() {
        builder.queryParams.put(paramName + "[desc]", "true");
        return builder;
      }
    }

    public enum InvoiceIdIsPresent {
      True("true"),

      False("false"),

      /**
       * An enum member indicating that InvoiceIdIsPresent was instantiated with an unknown value.
       */
      _UNKNOWN(null);
      private final String value;

      InvoiceIdIsPresent(String value) {
        this.value = value;
      }

      public String getValue() {
        return value;
      }

      public static InvoiceIdIsPresent fromString(String value) {
        if (value == null) return _UNKNOWN;
        for (InvoiceIdIsPresent enumValue : InvoiceIdIsPresent.values()) {
          if (enumValue.value != null && enumValue.value.equals(value)) {
            return enumValue;
          }
        }
        return _UNKNOWN;
      }
    }

    public enum SourceIs {
      AdminConsole("admin_console"),

      Api("api"),

      BulkOperation("bulk_operation"),

      /** An enum member indicating that SourceIs was instantiated with an unknown value. */
      _UNKNOWN(null);
      private final String value;

      SourceIs(String value) {
        this.value = value;
      }

      public String getValue() {
        return value;
      }

      public static SourceIs fromString(String value) {
        if (value == null) return _UNKNOWN;
        for (SourceIs enumValue : SourceIs.values()) {
          if (enumValue.value != null && enumValue.value.equals(value)) {
            return enumValue;
          }
        }
        return _UNKNOWN;
      }
    }

    public enum SourceIsNot {
      AdminConsole("admin_console"),

      Api("api"),

      BulkOperation("bulk_operation"),

      /** An enum member indicating that SourceIsNot was instantiated with an unknown value. */
      _UNKNOWN(null);
      private final String value;

      SourceIsNot(String value) {
        this.value = value;
      }

      public String getValue() {
        return value;
      }

      public static SourceIsNot fromString(String value) {
        if (value == null) return _UNKNOWN;
        for (SourceIsNot enumValue : SourceIsNot.values()) {
          if (enumValue.value != null && enumValue.value.equals(value)) {
            return enumValue;
          }
        }
        return _UNKNOWN;
      }
    }

    public enum SortByAsc {
      UsageDate("usage_date"),

      UpdatedAt("updated_at"),

      /** An enum member indicating that SortByAsc was instantiated with an unknown value. */
      _UNKNOWN(null);
      private final String value;

      SortByAsc(String value) {
        this.value = value;
      }

      public String getValue() {
        return value;
      }

      public static SortByAsc fromString(String value) {
        if (value == null) return _UNKNOWN;
        for (SortByAsc enumValue : SortByAsc.values()) {
          if (enumValue.value != null && enumValue.value.equals(value)) {
            return enumValue;
          }
        }
        return _UNKNOWN;
      }
    }

    public enum SortByDesc {
      UsageDate("usage_date"),

      UpdatedAt("updated_at"),

      /** An enum member indicating that SortByDesc was instantiated with an unknown value. */
      _UNKNOWN(null);
      private final String value;

      SortByDesc(String value) {
        this.value = value;
      }

      public String getValue() {
        return value;
      }

      public static SortByDesc fromString(String value) {
        if (value == null) return _UNKNOWN;
        for (SortByDesc enumValue : SortByDesc.values()) {
          if (enumValue.value != null && enumValue.value.equals(value)) {
            return enumValue;
          }
        }
        return _UNKNOWN;
      }
    }
  }
}
